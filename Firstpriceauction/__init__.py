...
class C(BaseConstants):
    ...
    EVALUATE_MIN = cu(0)
    EVALUATE_MAX = cu(100)
    INSTRUCTIONS_TEMPLATE = 'Firstpriceauction/instructions.html'
...
def set_payoff(player: Player):
    group = player.group
    if player.role_type=="buyer":
        if player.is_winner:
            player.payoff = player.item_value - player.bid_amount
        #if player.payoff < 0:
         #   player.payoff = 0
        else:
            player.payoff = 0
    ...
    if player.role_type=="seller":
        player.payoff=group.highest_bid
